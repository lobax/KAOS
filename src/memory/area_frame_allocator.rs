...
impl FrameAllocator for AreaFrameAllocator { 
    fn allocate_frame(&mut self) -> Option<Frame> { 
        if let Some(area) = self.current_area { 
            ...
            // Frame doesn't implement Clone, but we can construct an
            // identical frame. 
            ...
            if frame > current_area_last_frame { 
                // all frames of current area are used, 
                // switch to next area
                ...
                // `frame` is usedby the multiboot information structure
                self.next_free_frame = Frame { 
            ...
            // `frame` was not valid, try it again with updated
            // `next_free_frame`
            self.allocate_frame()
        } else { 
